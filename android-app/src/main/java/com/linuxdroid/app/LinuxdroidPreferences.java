...
final class LinuxdroidPreferences {
    ...
    LinuxdroidPreferences(Context context) {
    ...
    static void storeCurrentSession(Context context, TerminalSession session) {
        PreferenceManager.getDefaultSharedPreferences(context).edit().putString(LinuxdroidPreferences.CURRENT_SESSION_KEY, session.mHandle).apply();
    }

    static TerminalSession getCurrentSession(LinuxdroidActivity context) {
        String sessionHandle = PreferenceManager.getDefaultSharedPreferences(context).getString(LinuxdroidPreferences.CURRENT_SESSION_KEY, "");
    ...
    public void reloadFromProperties(Context context) {
        File propsFile = new File(LinuxdroidService.HOME_PATH + "/.linuxdroid/linuxdroid.properties");
        if (!propsFile.exists())
            propsFile = new File(LinuxdroidService.HOME_PATH + "/.config/linuxdroid/linuxdroid.properties");
        ...
        try {
            ...
            Toast.makeText(context, "Could not open the propertiey file linuxdroid.properties.", Toast.LENGTH_LONG).show();
            Log.e("linuxdroid", "Error loading props", e);
        }
    ...
    private void parseAction(String name, int shortcutAction, Properties props) {
        ...
        if (!(parts.length == 2 && parts[0].trim().equals("ctrl")) || input.isEmpty() || input.length() > 2) {
            Log.e("linuxdroid", "Keyboard shortcut '" + name + "' is not Ctrl+<something>");
            return;
        }
