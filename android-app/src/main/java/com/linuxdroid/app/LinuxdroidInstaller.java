...
/**
 * Install the Linuxdroid bootstrap packages if necessary by following the below steps:
 * <p/>
 ...
 */
final class LinuxdroidInstaller {

    /** Performs setup if necessary. */
    static void setupIfNeeded(final Activity activity, final Runnable whenDone) {
        // Linuxdroid can only be run as the primary user (device owner) since only that
        ...
        final File PREFIX_FILE = new File(LinuxdroidService.PREFIX_PATH);
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    final String STAGING_PREFIX_PATH = LinuxdroidService.FILES_PATH + "/usr-staging";
                        ...
                        try {
                            new AlertDialog.Builder(activity).setTitle(R.string.bootstrap_error_title).setMessage(R.string.bootstrap_error_body)
                                .setNegativeButton(R.string.bootstrap_error_abort, (dialog, which) -> {
                                    ...
                                    dialog.dismiss();
                                    LinuxdroidInstaller.setupIfNeeded(activity, whenDone);
                                }).show();
    ...
    static URL determineZipUrl() throws MalformedURLException {
        ...
        return new URL("https://github.com/linuxdroid/linuxdroid.github.io/raw/linuxdroid/bootstrap-" + archName + ".zip");
    }
    ...
    public static void setupStorageSymlinks(final Context context) {
        final String LOG_TAG = "linuxdroid-storage";
        new Thread() {
            public void run() {
                try {
                    File storageDir = new File(LinuxdroidService.HOME_PATH, "storage");
