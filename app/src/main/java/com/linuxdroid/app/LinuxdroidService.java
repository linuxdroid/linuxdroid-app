 ...
 * running so that it is not terminated. The user interacts with the session through {@link LinuxdroidActivity}, but this
 * service may outlive the activity when the user or the system disposes of the activity. In that case the user may
 * restart {@link LinuxdroidActivity} later to yet again access the sessions.
 * <p/>
...
public final class LinuxdroidService extends Service implements SessionChangedCallback {
    ...
    private static final String ACTION_STOP_SERVICE = "com.linuxdroid.service_stop";
    private static final String ACTION_LOCK_WAKE = "com.linuxdroid.service_wake_lock";
    private static final String ACTION_UNLOCK_WAKE = "com.linuxdroid.service_wake_unlock";
    /** Intent action to launch a new terminal session. Executed from LinuxdroidWidgetProvider. */
    ...
    public static final String EXTRA_CURRENT_WORKING_DIRECTORY = "com.linuxdroid.execute.cwd";
    private static final String EXTRA_EXECUTE_IN_BACKGROUND = "com.linuxdroid.execute.background";

    /** This service is only bound from inside the same process and never uses IPC. */
    class LocalBinder extends Binder {
        public final LinuxdroidService service = LinuxdroidService.this;
    }
     ...
     * <p/>
     * Note that this list is observed by {@link LinuxdroidActivity#mListViewAdapter}, so any changes must be made on the UI
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (ACTION_STOP_SERVICE.equals(action)) {
            ...
            if (intent.getBooleanExtra(EXTRA_EXECUTE_IN_BACKGROUND, false)) {
                ...
                LinuxdroidPreferences.storeCurrentSession(this, newSession);

                // Launch the main Linuxdroid app, which will now show the current session:
                startActivity(new Intent(this, LinuxdroidActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
            }
        } else if (action != null) {
            Log.e(EmulatorDebug.LOG_TAG, "Unknown LinuxdroidService action: '" + action + "'");
        }
    ...
    private Notification buildNotification() {
        Intent notifyIntent = new Intent(this, LinuxdroidActivity.class);
        ...
        Intent exitIntent = new Intent(this, LinuxdroidService.class).setAction(ACTION_STOP_SERVICE);
        builder.addAction(android.R.drawable.ic_delete, res.getString(R.string.notification_action_exit), PendingIntent.getService(this, 0, exitIntent, 0));

        String newWakeAction = wakeLockHeld ? ACTION_UNLOCK_WAKE : ACTION_LOCK_WAKE;
        Intent toggleWakeLockIntent = new Intent(this, LinuxdroidService.class).setAction(newWakeAction);
